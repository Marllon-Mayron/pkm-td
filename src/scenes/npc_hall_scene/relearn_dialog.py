...
Permite ao jogador fazer um Pokémon reaprender um golpe que ele
JÁ poderia ter aprendido até seu nível atual (não inclui golpes
futuros, apenas os que ele já "passou" pelo learnset).

Mostra descrição completa do golpe selecionado para ajudar o jogador
a tomar a decisão.
"""
...
class RelearnDialog:
    ...
    # Tamanhos base (multiplicados por escala responsiva)
    POKE_ROW_H_BASE = 62
    MOVE_ROW_H_BASE = 58
    ...
    def __init__(self, game, cost):
        ...
        self.learnable_moves = []       # [{name, info}]
    ...
    def _recalc_layout(self):
        ...
        # Janela grande para caber tudo com conforto
        w = int(ww * 0.94)
        h = int(wh * 0.90)
        x = (ww - w) // 2
        y = (wh - h) // 2
        ...
        # Proporções verticais da coluna direita
        # 42% golpes · 34% detalhes · 24% slots/botões
        gap_v = 12
        moves_h = int((right_h - gap_v * 2) * 0.44)
        detail_h = int((right_h - gap_v * 2) * 0.34)
        ...
        self._fonts = {
            ...
            'desc':     pygame.font.Font(None, int(base * 0.96)),
    ...
    def _refresh_learnable_moves(self):
        """Atualiza a lista de golpes reaprendíveis."""
        ...
        # Todos os golpes que o pokemon já poderia ter aprendido até o nível atual
    ...
    def _get_move_description(self, move_name, move_info=None):
        """Descrição completa do golpe (mesma lógica usada no modal de Pokémon)."""
    ...
    def _render_pokemon_list(self, screen):
        ...
        screen.blit(hdr_s, (hdr.x + 14, hdr.y + (hdr.height - hdr_s.get_height()) // 2))
        ...
        for i in range(start, end):
            ...
            screen.blit(lv_s, (row.right - lv_s.get_width() - 10,
                                row.y + 10))

            id_s = self._fonts['tiny'].render(
                f"#{entry['id']:04d}", True, (130, 140, 170))
            screen.blit(id_s, (row.right - id_s.get_width() - 10,
    ...
    def _render_moves_list(self, screen):
        ...
        for i in range(start, end):
            ...
            # Badges (tipo + categoria) embaixo do nome
        ...
        # Scrollbar
        if len(self.learnable_moves) > visible:
            bar_x = self.moves_rect.right - 7
            bar_top = self.moves_rect.y + 44
            bar_h = self.moves_rect.height - 48
            thumb_h = max(24, int(bar_h * visible / len(self.learnable_moves)))
    ...
    def _render_detail(self, screen):
        ...
        pygame.draw.line(screen, (70, 80, 105),
                         (hdr.x + 8, hdr.bottom - 1),
                         (hdr.right - 8, hdr.bottom - 1), 1)
        hdr_s = self._fonts['section'].render("DETALHES DO GOLPE",
                                                True, (200, 210, 235))
        ...
        if not mv:
            f = self._fonts['medium']
            s = f.render("Selecione um golpe para ver os detalhes",
                         True, (160, 170, 190))
            screen.blit(s, (self.detail_rect.centerx - s.get_width() // 2,
                            self.detail_rect.centery - s.get_height() // 2))
            return

        info = mv["info"]
        inner = pygame.Rect(self.detail_rect.x + 16,
                            self.detail_rect.y + 48,
                            self.detail_rect.width - 32,
                            self.detail_rect.height - 56)

        # ===== LINHA SUPERIOR: nome + stats principais =====
        # Nome
        name_s = self._fonts['title'].render(
            mv["name"].capitalize(), True, (255, 240, 200))
        screen.blit(name_s, (inner.x, inner.y + 4))

        # Badges de tipo e categoria (à direita do nome)
        badge_x = inner.x + name_s.get_width() + 16
        badge_y = inner.y + 8

        mv_type = info.get("type", "normal")
        type_color = self.pokedex.get_type_color(mv_type)
        type_s = self._fonts['small'].render(
            _sanitize(_PT_TYPE.get(mv_type, mv_type.upper())).upper(),
            True, (255, 255, 255))
        type_badge = pygame.Rect(badge_x, badge_y,
                                  type_s.get_width() + 16,
                                  type_s.get_height() + 6)
        pygame.draw.rect(screen, type_color, type_badge, border_radius=4)
        pygame.draw.rect(screen, (0, 0, 0), type_badge, 1, border_radius=4)
        screen.blit(type_s, (type_badge.x + 8, type_badge.y + 3))
        badge_x = type_badge.right + 8

        cat = info.get("category", "physical")
        cat_lbl, cat_color = _CATEGORY_PT.get(cat, ("---", (150, 150, 150)))
        cat_s = self._fonts['small'].render(cat_lbl, True, (255, 255, 255))
        cat_badge = pygame.Rect(badge_x, badge_y,
                                 cat_s.get_width() + 16,
                                 cat_s.get_height() + 6)
        pygame.draw.rect(screen, cat_color, cat_badge, border_radius=4)
        pygame.draw.rect(screen, (0, 0, 0), cat_badge, 1, border_radius=4)
        screen.blit(cat_s, (cat_badge.x + 8, cat_badge.y + 3))

        # ===== LINHA DE STATS (PWR / ACC / PP / EFEITO) =====
        stats_y = inner.y + name_s.get_height() + 16
        stat_items = [
            ("PODER", str(info.get("power") or "--"), (255, 175, 90)),
            ("PRECISÃO",
             f"{info.get('accuracy')}%" if info.get("accuracy") else "--",
             (170, 210, 255)),
            ("PP", str(info.get("pp") or "--"), (170, 220, 170)),
        ]

        stat_w = int(inner.width * 0.16)
        gap = 12
        sx = inner.x
        for label, value, color in stat_items:
            block = pygame.Rect(sx, stats_y, stat_w, 46)
            pygame.draw.rect(screen, (24, 28, 40), block, border_radius=6)
            pygame.draw.rect(screen, (60, 70, 95), block, 1, border_radius=6)

            lf = self._fonts['tiny'].render(label, True, (150, 160, 190))
            screen.blit(lf, (block.x + 10, block.y + 6))

            vf = self._fonts['detail_val'].render(value, True, color)
            screen.blit(vf, (block.x + 10, block.y + 6 + lf.get_height() + 2))

            sx += stat_w + gap

        # Efeito / chance à direita
        eff_rect = pygame.Rect(sx, stats_y,
                                inner.right - sx, 46)
        if eff_rect.width > 120:
            pygame.draw.rect(screen, (24, 28, 40), eff_rect, border_radius=6)
            pygame.draw.rect(screen, (60, 70, 95), eff_rect, 1, border_radius=6)

            lf = self._fonts['tiny'].render("EFEITO", True, (150, 160, 190))
            screen.blit(lf, (eff_rect.x + 10, eff_rect.y + 6))

            effect = info.get("effect") or "Nenhum"
            chance = info.get("effect_chance")
            eff_text = _sanitize(effect).replace("_", " ").title()
            if chance:
                eff_text += f" ({chance}%)"
            vf = self._fonts['small'].render(eff_text, True, (200, 210, 230))
            screen.blit(vf, (eff_rect.x + 10,
                             eff_rect.y + 6 + lf.get_height() + 2))

        # ===== DESCRIÇÃO COMPLETA =====
        desc_y = stats_y + 46 + 14
        pygame.draw.line(screen, (60, 70, 95),
                         (inner.x, desc_y), (inner.right, desc_y), 1)
        desc_y += 10

        lbl = self._fonts['tiny'].render("DESCRIÇÃO", True, (150, 160, 190))
        screen.blit(lbl, (inner.x, desc_y))
        desc_y += lbl.get_height() + 4

        desc = self._get_move_description(mv["name"], info)
        desc_font = self._fonts['desc']
        lines = self._wrap_text(desc, desc_font, inner.width)

        max_lines = max(1, (inner.bottom - desc_y) // (desc_font.get_height() + 2))
        for line in lines[:max_lines]:
            s = desc_font.render(line, True, (220, 225, 240))
            screen.blit(s, (inner.x, desc_y))
    ...
    def _render_slots(self, screen):
        ...
        for i, rect in enumerate(self.slot_buttons):
            ...
            # Número do slot (topo esquerdo)
            idx_s = self._fonts['tiny'].render(f"SLOT {i + 1}", True, (150, 160, 190))
            ...
            if has_move:
                ...
                type_s = self._fonts['tiny'].render(
                    _sanitize(_PT_TYPE.get(mv_type, mv_type.upper())).upper(),
                    True, (255, 255, 255))
                type_badge = pygame.Rect(rect.x + 16, rect.y + 26 + nm_s.get_height() + 4,
                                          type_s.get_width() + 12,
                                          type_s.get_height() + 4)
                pygame.draw.rect(screen, type_color, type_badge, border_radius=3)
                screen.blit(type_s, (type_badge.x + 6, type_badge.y + 2))

                # PP (canto inferior direito)
    ...
    def _render_buttons(self, screen):
        ...
        screen.blit(cs2, cs2.get_rect(center=self.cancel_btn.center))# src/scenes/npc_hall_scene/relearn_dialog.py
"""
Diálogo do NPC_2 - Reaprender movimentos.
    ...
    def _render_comparison(self, screen):
        ...
        if self.target_slot < 0:
            ...
            if cur_mv is None:
                ...
                # Cria dict info compatível para o renderizador
                cur_info = {
